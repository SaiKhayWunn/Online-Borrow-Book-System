  ...
  <header >
    ...
    <div class="sidebar-fixed position-fixed elegant-color-dark">
      ...
      <a class="logo-wrapper waves-effect ">
        <img src="img/logo.png2.png" class="img-fluid" alt="#">
      </a>
...
<br>
<main class="pt-5 mx-lg-5 view "style="background-image: url('img/polygon-2560x1440-4k-hd-wallpaper-3d-241.jpg'); background-repeat: no-repeat; background-size: cover; background-position: center center;">
  <br>
<!--Modal: modalPush-->
<div class=" white-text table-responsive container">
<br>
  <h4 class="text-light"> New Borrow List </h4> <br>
...
WHERE b.borrowID=bd.borrowID AND b.borrowID=d.borrowID AND bn.bookID=bd.bookID AND b.memberID=m.memberID AND b.status=1 GROUP BY bd.borrowID ORDER BY b.borrowID DESC";
      ...
      <td>
         <form action="" method="post">
           ...
           <input type="hidden" name="bookid" value="<?php echo $row['bookID']; ?>">
            <input type="hidden" name="bqty" value="<?php echo $row['bqty']; ?>">
...
if(isset($_POST['action']))
{
  ...
  $del=mysqli_query($con,"UPDATE borrow SET status='$st' WHERE borrowID=$bid");
  if($del)
  {
    if($st==0)
    {
    echo "<script> alert('Cancel Successfully');
    location.assign('cancelborrow.php');</script>";
    }
    elseif($st==2)
    {
  foreach ($bt as $id => $tt)
